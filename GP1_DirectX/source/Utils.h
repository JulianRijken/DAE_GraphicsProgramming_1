...
namespace dae
{
	namespace Utils
	{
		...
		static bool ParseOBJ(ID3D11Device* devicePtr, const std::string& filename, std::vector<VertexModel>& vertices, std::vector<uint32_t>& indices, std::vector<std::string>& mappedMaterials, bool flipAxisAndWinding = true)
		{
			...
			// Starts with no indesx as usemtl has to be called first
			int materialIndex{ -1 };
		...
		static bool ParseMTL(ID3D11Device* devicePtr, const std::string& filename, std::map<std::string, Material*>& materials)
		{
			std::ifstream file(RESOURCES_PATH + filename);
			...
			while (!file.eof())
			{
				...
				else if (command == "map_Kd")
				{
					...
					std::cout << "Set Kd: " << texturePath << std::endl;
					materials[currentMaterialName]->diffuse = Texture::LoadFromFile(devicePtr, texturePath);
				}
				else if (command == "map_d")
				{
					...
					std::cout << "Set d: " << texturePath << std::endl;
					materials[currentMaterialName]->opacity = Texture::LoadFromFile(devicePtr, texturePath);
				}
